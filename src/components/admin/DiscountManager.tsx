...
export function DiscountManager({
  ...
  async function handleResyncStripe() {
    if (
      !confirm(
        "Re-create all discount codes on the current Stripe account? Use this after switching Stripe accounts.",
      )
    ) {
      return;
    }
    ...
    setError(null);
    setMessage(null);
    const res = await fetch("/api/admin/discounts/resync", { method: "POST" });
    ...
    const extra =
      data.errors && data.errors.length > 0
        ...
        : "";
    setMessage(
      `Synced ${data.synced ?? 0} of ${data.total ?? 0} codes to Stripe.${extra}`,
    );
    router.refresh();
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-start justify-between gap-3 rounded-lg border border-gray-200 bg-white p-5">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">
            Stripe promotion codes
          </h2>
          <p className="mt-1 text-sm text-gray-500">
            Checkout uses Stripe’s promo field. After switching Stripe accounts,
            resync so your admin codes exist on the new account.
          </p>
        </div>
        <button
          type="button"
          ...
          className="rounded bg-brand-green px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:opacity-50"
        >
          {resyncing ? "Syncing…" : "Resync codes to Stripe"}
        </button>
      </div>
      {message && (
